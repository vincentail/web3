...
contract OpenOraclePriceData is OpenOracleData {
    ...
    event decodeEvent(string key, uint64 value);
    ...
    function testDecodeOne(bytes calldata message) external {
        string memory key = abi.decode(message, (string));
        emit decodeEvent(key, uint64(0));
    }

    function testDecode(bytes calldata message) external {
        (string memory key, uint64 value) = abi.decode(
            message,
            (string, uint64)
        );
        emit decodeEvent(key, value);
    }
    ...
    {
        // console.log("put data message '%s' signature '%s'", message, signature);
        (
            address source,
            uint64 timestamp,
            string memory key,
        ...
        // console.log(
        //     "source %s timestamp %s key %s value %s",
        //     source,
        //     timestamp,
        //     key,
        //     value
